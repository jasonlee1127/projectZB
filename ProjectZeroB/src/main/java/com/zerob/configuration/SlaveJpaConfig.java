...
@EnableJpaRepositories(
		...
		basePackages = {"com.zerob.repository.slave"}
)
public class SlaveJpaConfig {
    ...
    public HikariConfig slaveHikariConfig() {
        ...
        config.setDataSourceProperties(addSlaveDataSourceProperties());
    ...
    @Primary
    public PlatformTransactionManager slaveTransactionManager(@Qualifier("slaveEntityManagerFactory") EntityManagerFactory emf) {
     ...
     * @return
     */
    @Bean(name = "slaveExceptionTranslation")
    @Primary
    public PersistenceExceptionTranslationPostProcessor slaveExceptionTranslation() {
    ...
    public LocalContainerEntityManagerFactoryBean slaveEntityManagerFactory() {
        ...
        factory.setPersistenceUnitName("slavePersistenceUnit");
        /**
         * Entity의 scan이며 해당 scan이 지정되어있지 않으면
         * persistence.xml 을 생성하여 hibernate의 연결 및 데이터 소스 관련된 부분에 대해 직접
         * xml 버전으로 파일을 생성해 주어야 한다.
         */
        factory.setPackagesToScan("com.zerob.entity.slave");
        
        HibernateJpaVendorAdapter adapter = new HibernateJpaVendorAdapter();
        adapter.setGenerateDdl(true);
        factory.setJpaVendorAdapter(adapter);
        factory.setJpaProperties(addSlaveHibernateProperties());
     ...
     * @return
     */
    @Bean(name = "addSlaveDataSourceProperties")
    @Primary
    Properties addSlaveDataSourceProperties() {
     ...
     * @return
     */
    @Bean(name = "addSlaveHibernateProperties")
    @Primary
    Properties addSlaveHibernateProperties() {
        Properties properties = new Properties();
